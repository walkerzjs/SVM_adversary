...
#ngram features and distances
def get_ngram_features(strategy_instance,X_train,y_train,ngram = 2):
    vectorizer = CountVectorizer(max_df=1.0, min_df=1, binary = True, ngram_range=(ngram,ngram), max_features=5720, lowercase=False)
    vectors_train = vectorizer.fit_transform(X_train)
    train_0_text = []
    for i in range(len(strategy_instance.class0)):
    #    X[i] = np.array(X[i])
        train_0_text.append(" ".join(strategy_instance.class0[i]))
    
    
    clf = strategy_instance.train_svm(parameters, vectors_train,y_train)
    features = vectorizer.get_feature_names()
#    train_0_vectors = vectorizer.transform(features)
#    print(train_0_vectors)
#    distances = clf.decision_function(train_0_vectors)
    coef = clf.coef_.toarray()
    pairs = list(zip(features,coef.tolist()[0]))
    pairs.sort(key = lambda x:x[1])
    return clf, pairs

clf1, pairs1 = get_ngram_features(strategy_instance,X_train,y_train,ngram=1)
clf2, pairs2 = get_ngram_features(strategy_instance,X_train,y_train,ngram=2)
clf3, pairs3 = get_ngram_features(strategy_instance,X_train,y_train,ngram=3)
clf4, pairs4 = get_ngram_features(strategy_instance,X_train,y_train,ngram=4)
clf5, pairs5 = get_ngram_features(strategy_instance,X_train,y_train,ngram=5)
clf6, pairs6 = get_ngram_features(strategy_instance,X_train,y_train,ngram=6)   
...
test_data_splitted_setList = []
for doc in test_data_splitted:
    doc_set = list(set(doc))
    test_data_splitted_setList.append(doc_set)


#doc_set = test_data_splitted_setList[89]
def ngram_permute_list(doc_set, pairs):
    diff_list = []
    for pair_ngram in pairs:
        diff = len(set(pair_ngram[0].split(" "))-set(doc_set))
        if diff==0 and pair_ngram[1]<0:
            diff_list.append([pair_ngram[0],diff,pair_ngram[1]])
...
for test_doc in test_data_splitted_modified:
    test_doc_set = list(set(test_doc))
#    print(test_doc_set)
    diff_list2 = ngram_permute_list(test_doc_set, pairs2)
    diff_list3 = ngram_permute_list(test_doc_set, pairs3)
    diff_list4 = ngram_permute_list(test_doc_set, pairs4)
    diff_list5 = ngram_permute_list(test_doc_set, pairs5)
    diff_list6 = ngram_permute_list(test_doc_set, pairs6)
    
    diff_list_all = diff_list2+diff_list3+diff_list4+diff_list5+diff_list6
    for term in diff_list_all:
        text = term[0]
...
#    X[i] = np.array(X[i])
    train_0_text.append(" ".join(train_0[i]))
